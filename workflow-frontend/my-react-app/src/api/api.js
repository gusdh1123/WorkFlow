...
// 환경변수에서 API 베이스 URL 가져오기, 없으면 로컬 기본값 사용
const BASE_URL = import.meta.env.VITE_API_BASE_URL || "http://localhost:8081";

// refresh는 반드시 refreshOnce()로만 호출
...
export function refreshOnce() {
  if (!refreshPromise) {
    refreshPromise = refreshClient
      .post("/api/refresh") // refreshToken 쿠키로 accessToken 재발급 요청
      .then((res) => {
        // 204: 비로그인 상태 (정상)
  ...
  return refreshPromise; // 이미 진행 중이면 기존 Promise 반환
}
...
let isRefreshing = false; // refresh 진행 상태 플래그
let queue = [];           // refresh 완료 전 대기 중인 요청들 저장
  ...
  async (error) => {
    ...
    if (isRefreshing) {
      return new Promise((resolve, reject) => {
        queue.push({
          resolve: (token) => {
            if (!token) return reject(error); // refresh 실패
            original.headers = original.headers || {};
            original.headers.Authorization = `Bearer ${token}`;
            resolve(api(original)); // 대기 요청 재실행
          },
          reject,
        });
      });
    ...
    try {
      const newToken = await refreshOnce(); // accessToken 재발급
      ...
      runQueue(e, null);
      setApiAccessToken(null); // 메모리 토큰 초기화
