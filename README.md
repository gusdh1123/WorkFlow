...
- 업무를 상태(TODO, IN_PROGRESS, DONE 등)로 구분하여 현재 진행 상황을 한눈에 파악할 수 있도록 설계했습니다.
- 역할(Role)에 따라 접근 권한을 나누어, 사용자별로 수행할 수 있는 작업을 구분했습니다.
- 업무 수정, 상태 변경 및 삭제 시 이력을 기록하여, 변경 내용을 확인하고 추적할 수 있도록 했습니다.
- Soft Delete와 Scheduler를 적용하여, 삭제된 데이터는 일정 기간 동안 복구할 수 있도록 하고, 이후에는 자동으로 완전히 삭제되도록 설계했습니다.
...
- 상태 전이 규칙을 Service 레벨에서 강제하여, Controller나 클라이언트 단에서의 우회로 잘못된 상태 변경이 발생하지 않도록 했습니다.
- 모든 변경 사항에 대해 reason 입력을 요구하여, 단순 변경 기록이 아닌 변경 의도까지 추적할 수 있도록 설계했습니다.
- Soft Delete + Scheduler 구조를 통해, 삭제 직후에는 복구가 가능하도록 하고, 일정 기간 이후에는 자동으로 정리되도록 하여 사용자 경험과 데이터 관리 효율을 모두 고려했습니다.
- 권한(Role)에 따라 조회 범위와 기능을 분리하여, 동일한 시스템에서도 사용자마다 다른 업무 흐름을 가지도록 설계했습니다.
