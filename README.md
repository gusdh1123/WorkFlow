...
📌 ﻿업무 상태 흐름과 권한 관리를 중심으로 설계한 협업 시스템입니다.

---

## ℹ️ 주제 선정 이유
권한 관리, 상태 전이, 변경 이력과 같은 실무적인 요소를 설계하고 구현해보고자 했습니다.
...
이를 기반으로 업무 관리 시스템을 설계하고 구현했습니다.

---

## 🎯 기획 및 설계 포인트

- 상태 전이 규칙을 통해 비정상적인 상태 변경 방지
- Role 기반 권한 분리로 사용자별 접근 범위 제어
- Soft Delete 기반 데이터 관리로 복구 및 유지보수 구조 확보
- 주요 데이터 변경 이력 로그 기록으로 추적 가능하도록 설계

---

## ⚠️ 트러블 슈팅

### Soft Delete 파일 누적 문제 해결
- Soft Delete 기반 복구 기능 적용 과정에서 삭제된 데이터 및 첨부 파일이 물리적으로 제거되지 않아 스토리지에 지속적으로 누적되는 문제가 발생<br>
→ 복구 가능성을 유지하면서 2주 경과 후 자동 삭제하는 Scheduler를 도입해 불필요한 파일이 누적되지 않도록 개선

### 상태 전이 로직 책임 분리
- 상태 전이 허용 여부 로직이 단일 Service에 집중되면서 조건문 증가로 인해 코드 복잡도가 높아지고 유지보수가 어려운 구조 발생<br>
→ 상태 전이 검증 로직을 분리하여 책임을 나누고 유지보수가 용이한 구조로 개선

### 동시성 문제
- 게시글 수정 과정에서 동시에 여러 사용자가 동일 데이터를 수정할 경우 이전 데이터로 덮어쓰는 동시성 문제가 발생<br>
→ 충돌 빈도가 낮은 수정 환경에서 비관적 락 대신 version 기반 낙관적 락을 적용해 락 경합 없이 처리했으며, 충돌 발생 시 사용자에게 새로고침 후 재시도를 안내하도록 구성
