...
- v1.1 예정
    - 글(업무) 휴지통 기능(첨부파일 포함, 2주 동안 복구 가능)
    - 즐겨찾기 기능(사용자 맞춤형 업무 편의성 향상)
...
- 복구 기능 추가 및 삭제 업무 조회 및 개선
    - 복구 버튼 추가
        ...
        - 업무 상세 페이지
    - 업무 복구 시 복구 사유 입력 필수
    - 복구 시 로그 기록 추가 (restore)
    - 복구 시 삭제 상태 값 true → false로 변경
    - 업무 삭제 로그 메시지를 *수정 → 삭제*로 명확히 변경
    - 삭제된 업무의 삭제 시간 빨간색 표시
    - 삭제된 업무는 삭제 시간 기준 정렬 (최근순 / 오래된순) 조회 가능
- 즐겨찾기
    - 즐겨찾기 전용 테이블 생성
    - 별 아이콘 클릭 시 노란색 활성화 (낙관적 업데이트 적용)
    - 리스트 / 상세 페이지에 즐겨찾기 UI 추가
    - 즐겨찾기 업무 정렬:
        - 즐겨찾기 시간 기준 (최근순 / 오래된순)
        - 기존 필터 조건 유지 적용
    - 대시보드에서 즐겨찾기 업무 조회 기능 추가
- 대시보드
    - 기존: 로그 조회만 가능
    - 변경: 업무 제목 클릭 시 해당 상세 페이지로 이동 가능
