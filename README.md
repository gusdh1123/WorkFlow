...
- ~~검색 및 필터 기능 강화: 업무 상태, 담당자, 기간, 키워드 등 다양한 조건으로 빠르고 정확한 업무 검색 지원~~(v1.2)
- 댓글 기능 추가: 업무별 댓글 작성, 수정, 삭제 기능 및 작성자 표시와 작성 시간 기록으로 원활한 협업 지원
- ~~알림 시스템: 상태 변경, 마감 임박 등 실시간 알림 기능 비동기 처리 및 실패 복구~~(v1.2)
...
- ~~운영 효율성 강화: 자동 삭제 스케줄러 개선, 로그 및 모니터링 체계 고도화~~(v1.2)
- 프로젝트, 팀 기능
    - 팀원 역할 및 협업 현황 관리
    - 업무 분배 및 진행 상태 시각화
