...
→ 복구 가능성을 유지하면서 2주 경과 후 자동 삭제하는 Scheduler를 도입해 불필요한 파일 누적을 방지

### 상태 전이 로직 책임 분리
- 상태 전이 허용 여부 로직이 단일 Service에 집중되면서 조건문 증가로 인해 코드 복잡도가 높아지고 유지보수가 어려운 구조 발생<br>
→ 상태 전이 검증을 별도 메서드로 분리하여 코드의 가독성과 유지보수성을 개선

### 동시성 문제
- 게시글 수정 과정에서 동시에 여러 사용자가 동일 데이터를 수정할 경우 이전 데이터로 덮어쓰는 동시성 문제가 발생<br>
→ 업무 관리 시스템의 특성을 고려해 비관적 락 대신 JPA `@Version`을 활용한 낙관적 락을 적용하여 락 경합 없이 처리하고, 충돌 발생 시 새로고침 후 재시도를 안내
