...
- 업무를 상태(TODO, IN_PROGRESS, DONE 등)로 구분하여 현재 진행 상황을 한눈에 파악할 수 있도록 했습니다.
- 역할(Role)에 따라 접근 권한을 나누어, 사용자별로 수행할 수 있는 작업을 구분할 수 있도록 했습니다.
- 업무 수정, 상태 변경 및 삭제 시 이력을 기록하여, 변경 내용을 확인하고 추적할 수 있도록 했습니다.
- 삭제된 데이터는 일정 기간 동안 복구할 수 있도록 했습니다.

---

## 🧠 설계 포인트

- 업무 상태가 임의로 변경되지 않도록 상태 전이 규칙을 두는 구조를 설계했습니다.
- 업무 변경 이력을 기록하여 변경 내용을 추적할 수 있는 구조를 설계했습니다.
- 삭제 데이터의 복구와 관리 효율을 고려하여 Soft Delete 기반 구조를 설계했습니다.
- 권한(Role)에 따라 조회 범위와 기능을 분리하여 사용자별로 다른 업무 흐름을 가지도록 설계했습니다.

---

## ⚠️ 트러블 슈팅

- 초기에는 상태 전이 로직을 Controller에서 처리했지만, 우회 가능성이 있어 Service 레벨에서 검증하도록 구조를 변경했습니다.
- 초기에는 변경 이력만 기록했지만, 변경 이유를 알 수 없다는 문제가 있어 reason 입력을 필수로 추가했습니다.
- Soft Delete만 적용할 경우 데이터가 계속 누적되는 문제가 있어, Scheduler를 추가하여 일정 기간 이후 자동으로 삭제되도록 개선했습니다.
